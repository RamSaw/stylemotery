...
def count_keywords(mod):
    ...
    codestr = cg.codegen.to_source(mod2)
...
def ngrammify_keywords(mod, n, keyxform=lambda x: x):
    rv = Counter(map(tuple, ngramiterator.ngrams(keyword_tokenize(mod), n)))
...
def ngrammify_ast(tree, n, keyxform=lambda x: x):
    ...
    rv = Counter(
        reduce(lambda x, y: x + y, map(lambda x: ngramiterator.ngrams(x, n), [tuple(map(str, p)) for p in paths])))
    return Counter(dict((keyxform(x), y) for x, y in rv.items()))
...
if __name__ == '__main__':
    try:
        thefile = os.path.join(R"C:\Users\bms\PycharmProjects\stylemotery_code\dataset700",
                               os.listdir(R"C:\Users\bms\PycharmProjects\stylemotery_code\dataset700")[0]) #sys.argv[1]
    except IndexError:
