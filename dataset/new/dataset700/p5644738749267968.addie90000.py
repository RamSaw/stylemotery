...
def solve_case(naomi_blocks, ken_blocks, case_number):
    ...
    while len(naomi_blocks_for_deceitful) > 0:
        ...
        ken_candidate = [x for x in ken_blocks_for_deceitful if x > naomi_call]
    ...
    while len(naomi_blocks) > 0:
        ...
        ken_candidate = [x for x in ken_blocks if x > naomi_call]
        if len(ken_candidate) > 0:
...
for case_number in range(1, int(total_cases) + 1):
    r_file.readline()
    n_values = list(map(float, r_file.readline().split(' ')))
    k_values = list(map(float, r_file.readline().split(' ')))
