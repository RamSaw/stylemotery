...
def transpose(grid):
    return list(map(list, list(zip(*grid))))
...
def find_grid(R, C, M):
    ...
    * The remaining cases are M <= R-2, and M + 1 == R == C >= 4, which are both solvable
    ...
    if M >= C:
        ...
        return s and s + [['*' for c in range(C)]]
    elif M <= R-2 and C >= 3:
        #print "M+1<R", R, C, M
        grid = [['.' for c in range(C)] for r in range(R)]
        grid[0][0] = 'c'
...
if __name__ == '__main__':
    ...
    with fin, fout:
        ...
        for case in range(1, T+1):
            r, c, m = list(map(int, fin.readline().split()))
            soln = solve(r, c, m)
            print("Case #{0}:\n{1}".format(case, soln), file=fout)
