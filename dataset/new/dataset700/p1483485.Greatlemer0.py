...
def generate_map():
    ...
    letter_map = {}
    for english, googlerese in list(known_mappings.items()):
        pairs = list(zip(english, googlerese))
        for e,g in pairs:
    ...
    if len(letter_map) == 26:
        ...
        for letter in all_letters:
            if not e_letter and letter not in list(letter_map.keys()):
                e_letter = letter
            if not g_letter and letter not in list(letter_map.values()):
                g_letter = letter
        letter_map[e_letter] = g_letter
    return "".join(list(letter_map.keys())), "".join(list(letter_map.values()))
