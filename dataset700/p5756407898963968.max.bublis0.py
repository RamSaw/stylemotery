...
if __name__ == "__main__":
    ...
    for i in range(1, num_of_cases + 1):
        ...
        first_arrangement = [list(map(int, sys.stdin.readline().strip().split())) for x in range(4)]

        second_answer = int(sys.stdin.readline().strip())
        second_arrangement = [list(map(int, sys.stdin.readline().strip().split())) for x in range(4)]
