...
def genBoards(R, C, M):
    ...
    for mines in combinations( product(list(range(R)), list(range(C))), M):
...
for caseNo in range(1, int(eval(input()))+1):
    R, C, M = list(map(int, input().split()))
    print(caseNo, R, C, M, file=sys.stderr) #, oneClickSolution(R, C, M)<>'Impossible'
    print('Case #%d:' % caseNo)  
    print(oneClickSolution(R, C, M))
    
print('time= %.1f seconds' % (clock()-clk ), file=sys.stderr)
