...
def parse(infile):
    ...
    for i in range(n):
        heights[i, :] = np.array(list(map(int, infile.readline().split())))
    return n, m, heights
...
def runmain():
    ...
    for iCase, args in myCases:
        ...
        print('Case #' + str(iCase) + ':', answer, file=outfile)
...
class Categorizer(dict):
    ...
    def PrintRanking(self, n=None):
        ...
        maxkey = max(len(str(key)) for key in self.keys())
        maxval = max(len(str(len(val))) for val in self.values())
    ...
    def Median(self):
        tot = 0
        for value in self.values(): tot += len(value)
        keys = list(self.keys())
        keys.sort()
        nCount = 0
        for key in keys:
    ...
    def Mode(self):
        return max(iter(self.items()), key=lambda x: len(x[1]))[0]
...
class Counter(dict):
    ...
    def PrintRanking(self, n=None):
        ...
        maxkey = max(len(str(key)) for key in self.keys())
        maxval = max(len(str(val)) for val in self.values())
        formatter = "{0:<" + str(maxkey) + "} {1:>" + str(maxval) + "}    {2}"
        for key, count in items[0:n]:
    ...
    def Mode(self):
        return max(iter(self.items()), key=lambda x: x[1])[0]
