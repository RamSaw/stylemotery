...
def solve_problem(matrix):
    ...
    lines_combo = (matrix[i] for i in range(4))
    rows_combo = ((matrix[k][i] for k in range(4)) for i in range(4))
    diagonals = ((matrix[i][i] for i in range(4)),
                 (matrix[i][3 - i] for i in range(4)))
