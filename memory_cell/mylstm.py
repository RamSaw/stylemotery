...
class LSTMBase(link.Chain):
    ...
    def __init__(self, in_size, out_size=None, lateral_init=None,
        ...
        with self.init_scope():
            self.upward = convert_with_weight_normalization(linear.Linear,in_size, 4 * out_size, initialW=0)
            self.lateral = convert_with_weight_normalization(linear.Linear,out_size, 4 * out_size, initialW=0,
                                         nobias=True)
