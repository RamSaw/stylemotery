...
for case in range(T) :
    A, B = list(map(int, f.readline()[:-1].split()))
    a = int(math.ceil(A**0.5))
    b = int(B**0.5)
    res = len([x for x in findPalindromeInRange(a, b) if isPalindrome(x**2)])
