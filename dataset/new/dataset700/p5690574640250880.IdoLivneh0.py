...
def conj_mat(a):
	R = len(a)
	C = len(a[0])
	res = [['.' for _ in range(R)] for __ in range(C)]
	for i in range(R):
		for j in range(C):
			res[j][i] = a[i][j]
	return res
...
def three_line_builder(R, C, M):
	res = [['.' for _ in range(C)] for __ in range(R)]
	res[-1][-1] = 'c'
	m = min([M, R * C - 9])
	stop_flag = False
	for j in range(C):
...
def over_three_line_builder(R, C, M):
	if M <= (R - 3) * C:
		res = [['*' for _ in range(C)] for __ in range(M / C)]
		flag = False
		if (M % C) != (C - 1):
			line = ['*' for _ in range(M % C)]
			line.extend(['.' for _ in range(C - (M % C))])
		else:
			line = ['*' for _ in range((M % C) - 1)]
			line.extend(['.' for _ in range((C - (M % C)) + 1)])
			flag = True
		res.append(line)
		index = len(res)
		res.extend([['.' for _ in range(C)] for __ in range(R - len(res))])
		if flag:
			res[index][0] = '*'
		res[-1][-1] = 'c'
		assert len(res) == R
		...
		res = [['*' for _ in range(C)] for __ in range(R - 3)]
		M -= (R - 3) * C
